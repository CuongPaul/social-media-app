...
const changeAdminController = async (req, res) => {
  ...
  try {
    ...
    if (!members.includes(new_admin)) {
      return res
        .status(400)
        .json({ message: "This user doesn't exist in group" });
    }
...
const leaveChatRoomController = async (req, res) => {
  ...
  try {
    ...
    if (userId == chatRoom.admin) {
      return res
        .status(400)
        .json({ message: "Can't leave group when you're admin" });
    }

    await chatRoom.updateOne({ $pull: { members: userId } });
    await User.findByIdAndUpdate(userId, {
      $pull: { chat_rooms: { _id: chatRoomId } },
    });

    return res
      .status(200)
      .json({ message: `You have left the group ${chatRoom.name}` });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
const createChatRoomController = async (req, res) => {
  ...
  try {
    ...
    const membersId = users.reduce(
      (acc, cur) =>
        cur.block_users.includes(userId) ? acc : acc.concat(cur._id),
      []
    );
  ...
  try {
    ...
    for (const memberId of members) {
      if (memberId != userId) {
        ...
        if (sockets.length) {
          sockets.forEach((socketId) => {
            req.io.sockets.to(socketId).emit("delete-chatroom", {
              notification,
              chat_room_id: chatRoomId,
            });
          });
...
const searchChatRoomsController = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "success", data: { count, rows: chatRoomsData } });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
const getChatRoomsByUserController = async (req, res) => {
  ...
  try {
    ...
    for (const chatRoom of chatRooms) {
      ...
      if (index != -1) {
        ...
        if (furthest_unseen_message) {
          const furthestUnseenMessage = await Message.findById(
            furthest_unseen_message
          );
      ...
      if (
        ...
        if (
          reciver.block_users.some((item) => String(item) == String(userId))
        ) {
          chatRoom.type_block = "is_blocked";
        }
        if (
          sender.block_users.some((item) => String(item) == String(reciver._id))
        ) {
    ...
    return res
      .status(200)
      .json({ message: "success", data: { count, rows: result } });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
    ...
    if (name != chatRoomName) {
      for (const memberId of members.map((item) => item._id)) {
        if (memberId != userId) {
          ...
          if (sockets.length) {
            sockets.forEach((socketId) => {
              req.io.sockets.to(socketId).emit("update-chatroom", {
                notification,
...
const removeMembersFromChatRoomController = async (req, res) => {
  ...
  try {
    ...
    const membersRemovedValid = [
      ...new Set(membersRemoved.map((item) => item._id)),
    ];
...
const createChatRoomForTwoPeopleController = async (req, res) => {
  ...
  try {
    ...
    if (reciver.block_users.includes(userId)) {
      return res
        .status(400)
        .json({ message: "Can't send message to this user" });
    }
