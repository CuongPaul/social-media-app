...
export const sendMessageToFriend = async (req, res) => {
    ...
    if (!text && !image) {
        return res.status(422).json({ error: "Don`t send empty message type something" });
    }
    try {
        ...
        const getChat = await Chat.findById(saveChat.id).populate("sender").populate("receiver");
        const chatdata = {
            id: saveChat.id,
        ...
        if (getChat.receiver.socketId) {
            req.io.to(getChat.receiver.socketId).emit("new-message", { data: chatdata });
        }
    } catch (err) {
        console.log(err);
