...
const searchUsersController = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "success", data: { count, rows: users } });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
const unblockUserController = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: `You unblocked ${userBlocked.name}` });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
  ...
  try {
    ...
    if (user.cover_image) {
      const pathName = decodeURIComponent(
        user.cover_image.split("/o/")[1].split("?alt=")[0]
      );
...
const getRecommendUsersController = async (req, res) => {
  ...
  try {
    ...
    for (const request of friendRequests) {
      const indexOfSender = recommendUserIds.findIndex(
        (item) => item == request.sender
      );
      if (indexOfSender !== -1) {
        recommendUserIds.splice(indexOfSender, 1);
      }

      const indexOfReceiver = recommendUserIds.findIndex(
        (item) => item == request.receiver
      );
    ...
    const query = {
      _id: {
        $in: recommendUserIds.filter(
          (item) =>
            !user.friends.some((element) => String(item) == String(element._id))
        ),
      },
    };
    ...
    return res
      .status(200)
      .json({ message: "success", data: { count, rows: recommendUsers } });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
