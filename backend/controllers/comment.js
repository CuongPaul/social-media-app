...
const deleteCommentController = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json({ message: "You don't allow delete this comment" });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
const updateCommentController = async (req, res) => {
  ...
  try {
    ...
    if (!comment) {
      return res
        .status(400)
        .json({ message: "You don't allow edit this comment" });
    }
    ...
    return res.status(200).json({
      message: "success",
      data: { _id, post, user, text, image, react },
    });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
...
const getCommentsByPostController = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "success", data: { count, rows: comments } });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
