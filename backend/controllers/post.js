...
const removeTagController = async (req, res) => {
  ...
  try {
    ...
    if (!tag_friends.includes(userId)) {
      return res
        .status(400)
        .json({ message: "You haven't been tagged in the post" });
    }
    ...
    return res
      .status(200)
      .json({ message: "You've removed the tag from post" });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
const updatePostController = async (req, res) => {
  ...
  if (old_images) {
    old_images =
      typeof old_images == "string" ? JSON.parse(old_images) : old_images;
  }

  try {
    const post = await Post.findOne({ user: userId, _id: postId }).populate(
      "user",
      {
        name: 1,
      }
    ...
    const imageDeleted = post.images.filter(
      (item) => !old_images.includes(item)
    );
    ...
    return res
      .status(200)
      .json({ data: postUpdated, message: "Update post successfully" });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
const getAllPostsController = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "success", data: { count, rows: posts } });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
const getPostsByUserController = async (req, res) => {
  ...
  try {
    ...
    const query = {
      $or: [
        { _id: { $in: postsIdTagOwner } },
        { user: ownerId, privacy: "PUBLIC" },
      ],
    };
