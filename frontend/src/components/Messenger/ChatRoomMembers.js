...
const ChatRoomMembers = ({ isOpen, setIsOpen }) => {
  ...
  const handleSearchMembers = (searchValue) => {
    ...
    setMembers(
      chatRoomSelected?.members.filter((item) => regex.test(item.name))
    );
  };
  ...
  const handleClickMemberItem = (member) => {
    if (currentUser._id === chatRoomSelected?.admin) {
      if (currentUser._id !== member._id) {
        const isSelected = membersSelected.findIndex(
        ...
        if (isSelected === -1) {
          ...
          setMembersSelected(
            membersSelected.filter((item) => item._id !== member._id)
          );
        }
      }
    } else {
      ...
      <DialogContent>
        {membersSelected.length ? (
          ...
          >
            ...
            >
              {membersSelected.map((friend) => (
                <Chip
                  ...
                  onDelete={() =>
                    setMembersSelected(
                      membersSelected.filter((item) => item._id !== friend._id)
                    )
                  }
                />
              ))}
            ...
            <Button
              ...
              disabled={
                chatRoomSelected.members.length - membersSelected.length < 3
              }
            >
              Remove
            </Button>
        ...
        <div style={{ display: "flex", marginBottom: "32px" }}>
          <TextField
            ...
            onKeyPress={(e) =>
              e.key === "Enter" && handleSearchMembers(searchValue)
            }
            InputProps={{
