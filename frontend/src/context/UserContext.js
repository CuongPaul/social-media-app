...
const UserReducer = (state, action) => {
  switch (action.type) {
    ...
    case "SET_CURRENT_USER":
      const recentAccountStored =
        JSON.parse(localStorage.getItem("recent_accounts")) || [];
      ...
      if (indexOfCurrentAccount === -1) {
        ...
        localStorage.setItem(
          "recent_accounts",
          JSON.stringify(recentAccountStored)
        );
      }
      if (
    ...
    case "ADD_FRIEND_ONLINE":
      ...
      const indexOfFriendOnlineAdded =
        friendsOnlineAfterAddFriendsOnline.findIndex(
          (item) => item._id === action.payload._id
        );
      if (indexOfFriendOnlineAdded === -1) {
        friendsOnlineAfterAddFriendsOnline.push(action.payload);
      } else {
        friendsOnlineAfterAddFriendsOnline[indexOfFriendOnlineAdded] =
          action.payload;
      }
    ...
    case "ACCEPT_FRIEND_REQUEST":
      const incommingFriendRequestsAfterAccept = [
        ...state.incommingFriendRequests,
      ].filter((friendRequest) => friendRequest._id !== action.payload);
    ...
    case "CANCEL_FRIEND_REQUEST":
      const sendedFriendRequestsAfterCancel = [
    ...
    case "REMOVE_RECENT_ACCOUNT":
      ...
      localStorage.setItem(
        "recent_accounts",
        JSON.stringify(recentAccountsAfterRemove)
      );

      return { ...state, recentAccounts: recentAccountsAfterRemove };

    case "DECLINE_FRIEND_REQUEST":
      const incommingFriendRequestsAfterDecline = [
        ...state.incommingFriendRequests,
      ].filter((friendRequest) => friendRequest._id !== action.payload);
    ...
    case "ADD_INCOMMING_FRIEND_REQUEST":
      const incommingFriendRequestsAfterAdd = Object.values(
        [...state.incommingFriendRequests, ...action.payload].reduce(
          (acc, cur) => {
    ...
    case "ADD_SOCKET_FOR_FRIEND_ONLINE":
      ...
      const indexOfFriendOnlineAddedSocket =
        friendsOnlineAfterAddSocket.findIndex(
          (item) => item._id === action.payload._id
        );
