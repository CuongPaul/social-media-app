  ...
  switch (action.type) {
    ...
    case "REACT_POST":
      ...
      if (postSelectedAfterReact) {
        const indexOfReact = postSelectedAfterReact.react[
          action.payload.key
        ...
        if (indexOfReact === -1) {
          ...
          postSelectedAfterReact.react[action.payload.key].splice(
            indexOfReact,
            1
          );
        }
        ...
        if (indexOfPostSelected !== -1) {
          ...
          if (indexOfReact === -1) {
            ...
            postsAfterReact[indexOfPostSelected].react[
              action.payload.key
            ].splice(indexOfReact, 1);
          }
        }
      }
      ...
      return {
        ...state,
        posts: postsAfterReact,
        postSelected: postSelectedAfterReact,
      };
    ...
    case "CREATE_POST":
      ...
      return {
        ...
        postsTotal:
          action.payload.privacy === "ONLY_ME"
            ? state.postsTotal
            : state.postsTotal + 1,
      };
    ...
    case "REACT_COMMENT":
      ...
      if (indexOfUserReact === -1) {
        commentsAfterReact[indexOfCommentReacted].react[
          action.payload.key
        ].push({
          _id: action.payload.user._id,
          name: action.payload.user.name,
