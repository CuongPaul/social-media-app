  ...
  useEffect(() => {
    const getInitialData = async () => {
      try {
        ...
        const { data: currentUserData } = await callApi({
          url: "/user",
          method: "GET",
        });
        if (currentUserData) {
        ...
        if (friendsOnlineData) {
          userDispatch({
            ...
            payload: friendsOnlineData.rows,
          });
        }

        const { data: chatRoomsData } = await callApi({
          method: "GET",
          url: "/chat-room",
        });
        if (chatRoomsData) {
  ...
  useEffect(() => {
    if (userState.currentUser?._id) {
      ...
      socketIO.current.on("add-socket-for-user-online", ({ _id, socket }) => {
        userDispatch({
          payload: { _id, socket },
          type: "ADD_SOCKET_FOR_FRIEND_ONLINE",
        });
      });
